...
class master:
    ...
    def __receive_booking_details(self, username, booking_id, car_id, conn):
        ...
        response = requests.get(
            "http://localhost:8080/api/booking/"+booking_id+"/"+username+"/"+car_id)
        data = response.json()
    ...
    def __receive_credentials(self, enc_password, username, conn):
        ...
        response = requests.post(
            "http://localhost:8080/api/login", {"email": username, "password": enc_password})
        print(response)
        data = response.json()
    ...
    def __receive_lock_details(self, lock_time, car_id, duration, conn):
        ...
        """
        response = "lock_details: "+ lock_time +car_id+str(duration)
        socket_utils.sendJson(conn, { "Response": response })
